...
int *criarMascara(NO* nota){
    ...
    for(int i = 0; i < tamanhoAlfabeto; i++){ // Inicializa a mascara
        mascara[i] = 0;
    }
    for(int i = 0; i < nota->plagioTamanho ; i++){
        mascara[nota->plagio[i]] |= (1 << i); // Cria a mascara
    } // cada posicao do vetor mascara vai ter um bit setado para 1, representando a ocorrencia de um elemento da string
    return mascara;
}
